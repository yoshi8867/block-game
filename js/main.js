...
var CONFIG = {
  ...
  // 기본 제한시간. 원작 실측 = 총 플레이 96.0초 − 보너스 20.1초 = 75.8/75.9초.
  // 보너스 2회를 합쳐 최대 96초가 된다.
  PLAY_SECONDS: 76,
  ...
  // 이 점수를 넘는 순간 시간을 더 준다. 원작 실측: 두 영상 모두 5,000 과 7,000
  // 에서 딱 한 번씩 뜬다 (6,860→8,920 은 7,000·8,000 을 함께 넘는데도 한 번뿐).
  // tools/score_audit.py 참조. 70 + 10 + 10 = 최대 90초.
  TIME_BONUS_AT: [5000, 7000],
  // 원작 실측도 정확히 +10.0초다. 게이지가 가득 차도 **버리지 않는다** —
  // 화면만 100%에서 잘리고 남은 시간은 그대로 쌓인다 (원작에서 넘친 만큼
  // 게이지가 4.5초간 가득 찬 채 머무는 것으로 확인). 우리도 같게 동작한다.
...
var Game = (function () {
  ...
  /** 시간을 더 준다. 게이지는 1을 넘지 않고 잠시 가득 찬 채로 있는다.
      원작대로 NICE!/PERFECT! 와 **동시에** 뜨고 소리도 겹쳐서 난다. */
  function timeBonus() {
    endAt += CONFIG.TIME_BONUS_SECONDS * 1000;
